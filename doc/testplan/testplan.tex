...
\newline
\textbf{How will we achieve this?}
The test will create a GeoGrid and passes this information to the writer which will create a JSON formatted output. The test will convert this JSON and get the contents of a file in string format and check if they are the same.
...
\newline
\textbf{How will we achieve this?}
The test will create a population and passes this information to the writer which will create a JSON formatted output. The test will convert this JSON and a comparison file in string format and check if they are the same.
...
\textbf{What will it test?}
This is a test that will examine if the JSON writer outputs a JSON formatted file and reads the exact information in JSON formats.\\
\newline
\textbf{How will we achieve this?}
The test creates a population and writes it to a JSON file which will be converted to a string to check with the expected output. Next, the same file is used to be read by the JSON reader and all values are inspected.
...
No tests will be made for the visualizer, as this is hard to test through code. The testing of QT will happen via simulations done by ourselves and outsiders checking if our interface is obvious and well made
\subsection{QT Proto}
...
\textbf{What will it test?}
This test will check whether the generator can handle an empty GeoGrid.\\
\newline
\textbf{How will we achieve this?}
...
\newline
\textbf{How will we achieve this?}
We will achieve this by passing a GeoGridConfig object that has no commuters and no workplace distribution configuration. After this we can check whether each poolsize is equal to the expected value.
...
\newline
\textbf{How will we achieve this?}
We will achieve this by passing a csv to the reader, followed by calling the generator and populator using the result of the previous steps. We will not verify if the populator worked as expected (as this is not changed within the context, it is just tested to ensure that it does not crash when using the data from the previous step), we will only check if the generated pools are within the expected bounds.
